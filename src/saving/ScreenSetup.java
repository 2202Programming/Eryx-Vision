...
public class ScreenSetup {
	...
	public static void setupScreen() {
		...
		/*System.out.println("\nMove the mouse to the box\nwhere it should type what to do...");
		stop(3);
		Point boxLocation=MouseInfo.getPointerInfo().getLocation();
		boxX=boxLocation.x;
		boxY=boxLocation.y;
		
		System.out.println("\nMove the mouse to the upper\nleft corner of where I should read...");
		stop(3);
		Point topLeftReadCorner=MouseInfo.getPointerInfo().getLocation();
		readStartX=topLeftReadCorner.x;
		readStartY=topLeftReadCorner.y;
		
		System.out.println("\nMove the mouse to the bottom\nright corner of where I should read...");
		stop(3);
		Point bottomRightReadCorner=MouseInfo.getPointerInfo().getLocation();
		readEndX=bottomRightReadCorner.x;
		readEndY=bottomRightReadCorner.y;
		*/
